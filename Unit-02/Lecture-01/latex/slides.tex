...
\begin{frame}{Core List Methods}
  ...
  \vspace{0.6em}
  \begin{itemize}[<+->]
    \item Methods mutate the list in place
  \end{itemize}
\end{frame}

\begin{frame}{Sorting and Reversing}
  \begin{itemize}[<+->]
    \item \texttt{sort()} modifies the list
    \item \texttt{sorted(list)} returns a new list
    \item \texttt{reverse()} reverses in place
  \end{itemize}
  \vspace{0.4em}
  \begin{lstlisting}[language=Python]
nums = [3, 1, 2]
...
\begin{frame}{List Comprehension}
  \begin{itemize}[<+->]
    ...
    \item Pattern: \texttt{[expression for item in iterable if condition]}
  \end{itemize}
  \vspace{0.4em}
  \begin{lstlisting}[language=Python]
...
\begin{frame}{Comprehension Examples}
  \begin{lstlisting}[language=Python]
nums = [3, 4, 7, 8, 10]
...
result = [n*n for n in nums if n % 2 == 0]
  \end{lstlisting}
  \begin{itemize}[<+->]
    \item Combine filtering and transformation
  \end{itemize}
\end{frame}

\begin{frame}{Worked Example: Marks Analyzer (Core Steps)}
  \begin{lstlisting}[language=Python]
...
passing = [m for m in marks if m >= pass_mark]
  \end{lstlisting}
  \begin{itemize}[<+->]
    \item Use \texttt{sorted(...)} to avoid mutating the original list
    \item Use a comprehension to filter values by a condition
  \end{itemize}
\end{frame}
...
\begin{frame}{Common Pitfalls}
  \begin{itemize}[<+->]
    \item Index out of range errors
    \item Confusing copy vs reference
    \item Modifying list while iterating
  \end{itemize}
\end{frame}

\begin{frame}{Pitfall: Aliasing vs Copy}
  \begin{lstlisting}[language=Python]
a = [1, 2, 3]
...
print(a)       # [1, 2, 3, 99]
  \end{lstlisting}
  \begin{itemize}[<+->]
    \item Fix: create a copy when you want an independent list
  \end{itemize}
  \begin{lstlisting}[language=Python]
b = a.copy()   # or: a[:]
  \end{lstlisting}
...
\begin{frame}{Checkpoint 2}
  Write a list comprehension to square only even numbers
  from \texttt{nums = [1,2,3,4,5,6]}.
\end{frame}
...
\begin{frame}{Summary}
  \begin{itemize}[<+->]
    ...
    \item Indexing and slicing enable flexible access
    \item Core methods modify lists in place
    \item Comprehensions create concise transformations
  \end{itemize}
\end{frame}
